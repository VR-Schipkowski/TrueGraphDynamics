...
class TemporalGraph:
    def __init__(self,nodes_file="../Data/OriginalData/users.tsv",follower_file="../Data/OriginalData/follows.tsv",truths_file="../Data/OriginalData/truths.tsv",time_intervall_file="../Data/OriginalData/truths.tsv",emotion_csv="../Data/ProcessedData/enriched_comments_emotions.tsv"):
        ...
        self.clean_nodes(True, False, False)
        _ = self.flag_users()
    ...
    def flag_users(self,bin=100):
        print("flagging users for posting false information ...")
        dangerous_user_count=0
        false_information_per_user=np.zeros(len(self.nodes_dict.keys()))
        for node_counter,node_id in enumerate(self.nodes_dict.keys()):
            ...
            for counter,truth_at_timestemp in enumerate(self.nodes_dict[node_id].truths_in_time_intervall):
                ...
                if false_clasification_counter>=2:
                    self.nodes_dict[node_id].truthfullness_flag_array[counter]=1 
            if sum(self.nodes_dict[node_id].truthfullness_flag_array)>=1:
                #print(f"{node_id}: posts bulshit")
                dangerous_user_count+=1
            if false_clasification_counter>=100:
                print(f"{node_id} post a lot of false statements({false_clasification_counter} false statements) the account name is;{self.nodes_dict[node_id].username}!!!!!")
            false_information_per_user[node_counter]=false_clasification_counter
        false_information_per_user = [x for x in false_information_per_user if x != 0]
        print(f"{dangerous_user_count} users posted false information")
        print(Counter(false_information_per_user))
        plt.figure()
        plt.xlim(0, 50)
        plt.hist(false_information_per_user, bins=bin)#
        plt.xlabel("False inormation count")
        plt.ylabel("Number of users")
        plt.title("Histogram")
        plt.show()
        return false_information_per_user